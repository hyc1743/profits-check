...
def test_alembic_upgrade_creates_expected_tables(tmp_path) -> None:
    ...
    daily_funding_asset_columns = {
        column["name"] for column in inspector.get_columns("daily_funding_fee_asset_summaries")
    }
    assert {
        "daily_summary_id",
        "channel_id",
...
def test_alembic_upgrade_adopts_existing_pre_alembic_database(tmp_path) -> None:
    ...
    with engine.connect() as connection:
        ...
        assert connection.scalar(text("select version_num from alembic_version")) == "20260621_0011"
