...
- **多渠道接入**：支持 Binance、Gate、OKX、Bitget、Bybit、Aster，以及 EVM 与 Bitcoin mainnet 链上钱包。
...
- 安装 Python 3.12（如果需要）
- 同步后端和前端依赖
- 当前端源码、配置或锁文件变化时执行 `bun run build` 构建前端静态文件
...
| `OKX_DEX_API_KEY` | 空 | OKX DEX API Key，用于读取链上钱包总估值 |
